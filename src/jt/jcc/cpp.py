...
from ..jvm.lib.compat import range
...
def typename(cls, current, const):
    ...
    return "const %s &" % (name) if const else name
...
def find_method(cls, name, params):
    ...
    if (Modifier.isAbstract(modifiers) or
...
def signature(fn, argsOnly=False):
    ...
    if argsOnly:
        return "(%s)" % (''.join([typename(param) for param in fn.getParameterTypes()]))
    else:
        return "(%s)%s" % (''.join([typename(param) for param in fn.getParameterTypes()]),
                           returnType)
...
def jcc(args):
    ...
    if recompile or not build and (install or dist or egg_info):
        ...
        if moduleName:
            with open(os.path.join(cppdir, moduleName) + ".cpp", "w") as out:
...
def header(env, out, cls, typeset, packages, excludes, generics,
    ...
    if cls.isInterface():
        superCls = interfaces.pop(0) if interfaces else findClass("java/lang/Object")
...
def code(env, out, cls, superCls, constructors, methods, protectedMethods,
    ...
    superClsName = superCls.getName() if superCls else "JObject"
    superNames = superClsName.split('.')
