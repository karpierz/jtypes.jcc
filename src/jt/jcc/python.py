...
from ..jvm.lib.compat import range, zip
...
from .config import VERSION as JCC_VER
...
def parseArgs(params, current, generics, genericParams=None):
    ...
    def checkarg(cls, genericPT=None):
        ...
        while cls.isArray():
            cls = cls.getComponentType()
        clsName = cls.getName()
        if cls.isPrimitive() or clsName in ("java.lang.String", "java.lang.Object"):
            return ""
        if is_boxed(clsName):
    ...
    return (sig, chk, ''.join([callarg(params[i], i) for i in range(len(params))]))
...
def construct(out, indent, cls, inCase, constructor, names, generics):
    ...
    typeParams = set() if generics else None
...
def call(out, indent, cls, inCase, method, names, cardinality, isExtension,
    ...
    if isExtension and name == 'clone' and Modifier.isNative(modifiers):
        ...
        line(out, indent, returnValue(cls, returnType, "result", genericRT, typeParams))
    else:
...
def jniargs(params):
    ...
    count = len(params)
    decls = ', '.join(['%s a%d' %(jniname(params[i]), i) for i in range(count)])
    if decls:
        return ', ' + decls
    else:
        return ''
...
def python(env, out_h, out, cls, superCls, names, superNames,
    ...
    clsParams = getTypeParameters(cls) if generics else None
    ...
    if clsParams:
        line(out, indent, 'static PyObject *t_%s_of_(t_%s *self, PyObject *args);', names[-1], names[-1])
    ...
    if sequenceLenMethod or sequenceGetMethod:
        ...
        line(out)
        line(out, indent, 'static PySequenceMethods t_%s_as_sequence = {', names[-1])
        ...
        if len(constructors) > 1:
            ...
            for constructor in constructors:
                ...
                if len(params) != currLen:
                    if (currLen > 0) if PY3 else (currLen >= 0):
                        withErr = True
...
def module(out, allInOne, classes, imports, cppdir, moduleName,
    ...
    if PY3:
        ...
        line(out, 1, "{")
        line(out, 2, 'PyObject *module = Py_InitModule3("%s", jcc_funcs, "");', extname)
    line(out)
...
def compile(env, jccPath, output, moduleName, install, dist, debug, jars,
    ...
    if shared:
        ...
        if find_jvm_dll:
            line(out, 1, "from jcc._platform import add_jvm_dll_directory_to_path")
            line(out, 1, "add_jvm_dll_directory_to_path()")
        line(out, 1, "import jcc")
    line(out, 0, "from . import %s", extname)
    line(out)
    line(out, 0, "__dir__ = os.path.dirname(os.path.abspath(__file__))")
    ...
    line(out, 1, 'pass')
    line(out)
    line(out, 0, '%s._set_exception_types(JavaError, InvalidArgsError)', extname)
